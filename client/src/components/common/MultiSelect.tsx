...
const MultiSelectDropdown = ({ options, setFilters }: MultiSelectDropdownProps) => {
  ...
  const handleChange = (event: SelectChangeEvent<string[]>) => {
    ...
    if (arrayValue.includes("Select All")) {
      ...
      const newCategories = (isAllSelected ? [] : allDisplayNames).map((contentValue) =>
        categoryOptions.find(option => option.content === contentValue)?.value
      ...
      const newCategories = arrayValue.map((contentValue: string) =>
        categoryOptions.find(option => option.content === contentValue)?.value
      ).filter((val): val is string => typeof val === 'string');
    ...
    <FormControl sx={{ m: 1, width: 150, background: "white" }} size="small" variant="outlined">
      ...
      >
        <MenuItem value="Select All">
          <Checkbox checked={displayName.length === allDisplayNames.length} indeterminate={displayName.length > 0 && displayName.length < allDisplayNames.length} />
