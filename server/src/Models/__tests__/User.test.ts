...
describe('UserModel', () => {
  ...
  it('should return a list of users from getUsers', async () => {
    ...
    expect(mockQuery).toHaveBeenCalledWith('SELECT id, username, email, created_at FROM users');
  });

  it('should return a user by ID from getUserById', async () => {
    const mockUser = { id: 1, username: 'user1', email: 'user1@example.com', created_at: new Date() };
  ...
  it('should create a new user and return it', async () => {
    const mockUser = { id: 1, username: 'user1', email: 'user1@example.com', hashed_password: 'hashed', created_at: new Date() };
    mockQuery.mockResolvedValue({ rows: [mockUser] });

    const result = await userModel.createUser('user1', 'user1@example.com', 'hashed');
  ...
  it('should return a user by email from loginUser', async () => {
    ...
    expect(result).toEqual(mockUser);
    expect(mockQuery).toHaveBeenCalledWith('SELECT * FROM users WHERE email=$1', ['user1@example.com']);
  });
  ...
  it('should delete a user by ID and return it', async () => {
    ...
    expect(mockQuery).toHaveBeenCalledWith('DELETE FROM users WHERE id=$1 RETURNING id', ['1']);
  });
  ...
  it('should return null if no user is found in getUserById', async () => {
    ...
    expect(mockQuery).toHaveBeenCalledWith('SELECT * FROM users WHERE id = $1', ['999']);
  });
});
